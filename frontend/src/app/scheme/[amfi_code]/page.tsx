...
export default function SchemeDetailsPage() {
    ...
    useEffect(() => {
        ...
        const fetchDetails = async () => {
            if (!userId) {
                // No user logged in - just view mode
                setIsOwnedScheme(false);
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                if (res) {
                    ...
                    // Scheme exists but not in portfolio
                    setIsOwnedScheme(false);
                }
            } catch (err: any) {
                // Scheme not in portfolio or error - still allow viewing enrichment data
                setIsOwnedScheme(false);
            } finally {
                setLoading(false);
            }
    ...
    // If scheme is not owned, show simplified view with enrichment data
    if (!isOwnedScheme || !data) {
        return (
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 bg-transparent">
                ...
                {/* Simplified Header for non-owned schemes */}
                <div className="bg-white/90 dark:bg-slate-900/50 backdrop-blur-md rounded-3xl p-8 mb-8 border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-xl">
                    ...
                    <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight leading-tight mb-2">
                        Scheme Analysis
                    </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
                <div className="col-span-1 sm:col-span-2 md:grid-cols-3 lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950/80 rounded-2xl p-6 border border-slate-200 dark:border-white/5 shadow-md flex flex-col justify-between relative overflow-hidden block">
                        ...
                        <div className="relative z-10">
                            {kpis.xirr_status === 'VALID' && kpis.xirr != null ? (
